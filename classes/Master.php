...
Class Master extends DBConnection {
	...
	function save_appointment(){
		...
		if(!in_array(strtolower(date("l",strtotime($date_sched))),explode(',',strtolower($sched_set['day_schedule'])))){
			$resp['status'] = 'failed';
			$resp['msg'] = "O dia da semana da programação selecionado é inválido.";
			return json_encode($resp);
			exit;
		}
		if(!( (strtotime($sched_time) >= strtotime($morning_start) && strtotime($sched_time) <= strtotime($morning_end)) || (strtotime($sched_time) >= strtotime($afternoon_start) && strtotime($sched_time) <= strtotime($afternoon_end)) )){
			$resp['status'] = 'failed';
			$resp['msg'] = "O horário de agendamento selecionado é inválido.";
			return json_encode($resp);
			exit;
		}
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "O agendamento selecionado entra em conflito com outro compromisso.";
			return json_encode($resp);
			exit;
		}
		if(empty($patient_id))
		...
		if($save_inv){
			...
			if(empty($id))
			$sql = "INSERT INTO `appointments` set date_sched = '{$date_sched}',patient_id = '{$patient_id}',`status` = '{$status}',`procedure` = '{$procedure}' ";
			else
			$sql = "UPDATE `appointments` set date_sched = '{$date_sched}',patient_id = '{$patient_id}',`status` = '{$status}',`procedure` = '{$procedure}' where id = '{$id}' ";

			$save_sched = $this->conn->query($sql);
			$this->capture_err();
			$data = "";
			foreach($_POST as $k=> $v){
				if(!in_array($k,array('lid','date_sched','status','procedure'))){
					if(!empty($data)) $data .=", ";
					$data .= " ({$patient_id},'{$k}','{$v}')";
				}
			}
			...
			if($save_sched && $save_meta){
				$resp['status'] = 'success';
				$resp['name'] = $name;
				$this->settings->set_flashdata('success',' Agendamemto salvo com sucesso');
			}else{
				$resp['status'] = 'failed';
				$resp['msg'] = "Ocorreu um erro ao enviar os dados.";
			}
	...
	function multiple_action(){
		...
		if($process){
			...
			$this->settings->set_flashdata("success","Agendamento(s) realizado(s) com sucesso ".$act);
		}else{
			$resp['status'] = 'failed';
			$resp['error_sql'] = $sql;
		}
	...
	function save_sched_settings(){
		...
		if($save){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',' Configurações de agendamento atualizadas com sucesso');
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error;
			$resp['msg'] = "Ocorreu um erro ao executar a consulta.";
